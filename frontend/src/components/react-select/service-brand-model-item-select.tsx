  ...
  > = async (searchQuery, _loadedOptions, additional = { page: 1 }) => {
    ...
    try {
      const response = await mainInstance.get(
        `/select/service-brand-items?page=${page}&search=${searchQuery}&sort=id&service_brand_model_id=${serviceBrandCategoryId}`,
      );

      const options = response.data.records.map((item: ServiceBrandModelItem) => ({
        value: item.id,
        label: item.service_item?.label,
        price: item.price
      }));
