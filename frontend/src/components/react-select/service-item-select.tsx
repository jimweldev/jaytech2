...
const ServiceItemSelect = ({ serviceBrandCategoryId, ...props }: any) => {
  ...
  > = async (searchQuery, _loadedOptions, additional = { page: 1 }) => {
    ...
    try {
      const response = await mainInstance.get(
        `/select/service-items?page=${page}&search=${searchQuery}&sort=id&service_brand_category_id=${serviceBrandCategoryId}`,
      );
