...
const UpdateBrandDialog = ({
  ...
  const { selectedServiceBrand } = useServiceBrandStore();
  ...
  useEffect(() => {
    if (selectedServiceBrand) {
      ...
      if (selectedServiceBrand.thumbnail_path) {
        setCroppedImage(null);
      }
    }
  }, [selectedServiceBrand, form]);
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    toast.promise(
      mainInstance.post(
        `/services/brands/${selectedServiceBrand?.id}?_method=PATCH`,
        formData,
      ),
      {
        loading: 'Updating...',
